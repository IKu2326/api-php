...
class controladorCliente {
    ...
    public static function crear() {
        ...
        if ($registro === "Cliente_Duplicado") {
            http_response_code(409); 
            echo json_encode(["mensaje" => "El Cliente ya está registrado."]);
        } elseif ($registro === true) {
            echo json_encode(["mensaje" => " creado exitosamente."]);
        } else {
            http_response_code(500); 
            echo json_encode(["mensaje" => "Error al registrar el cliente."]);
        }
    }
